...
function draw(){
 ...
 const splitGroups=groups.flatMap(g=>[{...g,name:g.name+' · 6 樓以下（含 6 樓）',accept:r=>r.floor<=6},{...g,name:g.name+' · 7 樓以上',accept:r=>r.floor>=7}]);
 const sets=splitGroups.map(g=>({...g,series:['兩房','三房'].map((room,i)=>({room,color:i?'#b35a16':'#087f85',points:months.map(month=>{
  const rows=records.filter(r=>g.towers.includes(r.tower)&&g.accept(r)&&r.rooms===room&&r.date.startsWith(month)&&Number.isFinite(r[key]));
 ...
 host.innerHTML=sets.map(g=>{
  ...
  for(const s of g.series){let segment=[];const flush=()=>{if(segment.length>1)svg+=`<polyline points="${segment.join(' ')}" fill="none" stroke="${s.color}" stroke-width="2"/>`;segment=[];};
   s.points.forEach((p,i)=>{if(!p){flush();return;}segment.push(`${x(i)},${y(p.value)}`);});flush();
   s.points.forEach((p,i)=>{if(p)svg+=`<circle cx="${x(i)}" cy="${y(p.value)}" r="5" fill="${s.color}" tabindex="0"><title>${p.month} ${s.room}：${fmt(p.value)}，${p.n} 筆</title></circle>`;});
  }
  svg+='</svg>';
  ...
  return `<section class="panel"><h3>${g.name}</h3><p class="hint">${key==='unit'?'縱軸未從 0 起算；所有分組使用相同刻度。':'總價含車位；所有分組使用相同刻度。'}</p><p class="trend-key"><span style="color:#087f85">● 兩房</span>　<span style="color:#b35a16">● 三房</span></p><div class="sale-trend">${svg}</div><details><summary>查看逐月數值與樣本數</summary><div class="table-wrap"><table><thead><tr><th>交易月份</th><th>兩房${label}</th><th>筆數</th><th>三房${label}</th><th>筆數</th></tr></thead><tbody>${rows}</tbody></table></div></details></section>`;
 }).join('');
}
