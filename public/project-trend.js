...
function draw(){
 ...
 const sets=groups.map(g=>({...g,series:['兩房','三房'].map((room,i)=>({room,color:i?'#b35a16':'#087f85',points:months.map(month=>{
  const rows=records.filter(r=>g.towers.includes(r.tower)&&r.rooms===room&&r.date.startsWith(month)&&Number.isFinite(r[key]));
  return rows.length?{month,n:rows.length,value:rows.reduce((s,r)=>s+r[key],0)/rows.length}:null;
 })}))}));
 const step=key==='unit'?10:1000,max=Math.ceil(Math.max(...sets.flatMap(g=>g.series.flatMap(s=>s.points.filter(Boolean).map(p=>p.value))))/step)*step;
 const x=i=>70+i/(months.length-1||1)*730,y=v=>280-v/max*230;
